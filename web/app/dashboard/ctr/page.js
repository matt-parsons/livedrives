...
import CtrMap from '../[business]/ctr/CtrMap';
import TrendChart from '../[business]/ctr/TrendChart';
import SessionList from '../[business]/ctr/SessionList';
import styles from '../[business]/ctr/styles.module.css';
...
export default async function CtrDashboardPage({ searchParams }) {
  ...
  if (session.role !== 'owner' && session.role !== 'admin') {
    redirect(`/dashboard?bId=${encodeURIComponent(business.id)}`);
  }

  const buildCtrHref = (nextOffset) => {
    ...
    return `/dashboard/ctr?${params.toString()}`;
  };
  ...
  const prevHref = buildCtrHref(offset + 1);
  const nextHref = offset > 0 ? buildCtrHref(offset - 1) : null;
