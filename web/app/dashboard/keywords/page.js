...
import GeoGridRunsSection from '../[business]/GeoGridRunsSection';
import KeywordPerformanceSpotlight from '../[business]/KeywordPerformanceSpotlight';
...
export default async function BusinessKeywordsPage({ searchParams }) {
  ...
  const runBaseHref = '/dashboard/runs';
  const buildRunHref = (runId) => {
    if (!runId) {
      return null;
    }

    const params = new URLSearchParams({ bId: String(business.id) });
    return `${runBaseHref}/${runId}?${params.toString()}`;
  };
  ...
  const keywordPerformance30d = Array.from(recentKeywordRuns.entries())
    .map(([key, entries]) => {
      ...
      return {
        ...
        latestRunHref: buildRunHref(latestRun.id),
  ...
  const geoGridTrendList = geoGridTrend.map((item) => ({
    ...
    latestRunHref: buildRunHref(item.latestRunId),
      ...
      <div className="dashboard-layout">
        ...
        <aside className="dashboard-layout__sidebar" aria-label="Workspace navigation">
          ...
          <div className="dashboard-sidebar__menu">
            <BusinessNavigation businessId={business.id} active="keywords" />
          </div>
        </aside>
