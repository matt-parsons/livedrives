...
async function createBusinessFromPreview(connection, { organizationId, userId, email }) {
  ...
  const dbPayload = mapToDbColumns(values);
  const columns = [...Object.keys(dbPayload), 'organization_id', 'created_at', 'updated_at'].join(', ');
  const placeholders = [...Object.keys(dbPayload).map(() => '?'), '?', 'NOW()', 'NOW()'].join(', ');

  const [businessResult] = await connection.query(
    `INSERT INTO businesses (${columns}) VALUES (${placeholders})`,
    [...Object.values(dbPayload), organizationId]
  );
...
export async function bootstrapUser(decoded) {
