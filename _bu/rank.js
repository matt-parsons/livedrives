...
(async () => {
  ...
  // 1) Pick origin
  const origin = await pickOrigin(config.origin_zones);
  ...
  // const { rank, totalReturned, matched, places } = await getPlacesApiRank({
  ...
  // note(`→ [API] rank@${origin.lat.toFixed(5)},${origin.lng.toFixed(5)}: ${rank}/${totalReturned} : ${matched?.name}`);
  // console.log(places);
  // // Log it in DB
  ...
  try {
    ...
    // 3) Run CTR
    while (attempts < 4) {
      rankResult = await getProfileRank({ runId, config, origin, keyword, sessionId, run2Captcha });
      console.log('');

      if (rankResult.reason === 'bad_ip') {
