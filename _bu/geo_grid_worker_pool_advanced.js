// geo_grid_worker_pool_advanced.js
...
if (!isMainThread) {
  ...
  parentPort.on('message', async (data) => {
    ...
    try {
      console.log(`[RANKING] Ranking point ${pointId} at lat ${lat}, lng ${lng}...`);
      console.log(`[RANKING] Point ${pointId} Inputs: Keyword='${keyword}', SoaxEndpoint='${soaxConfig?.endpoint ?? 'N/A'}'`);
      ...
      for (const run of runs) {
        ...
        await new Promise(resolve => {
            ...
            for (let i = 0; i < MAX_CONCURRENCY && tasks.length > 0; i++) {
              const worker = new Worker(path.join(__dirname, 'geo_grid_worker_pool_advanced.js'), {
