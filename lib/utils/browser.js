...
async function prepareMobilePage(context, page, device, origin, fingerprintProfile = null) {
  const userAgent = fingerprintProfile?.userAgent || device.userAgent;
  const viewport = fingerprintProfile?.screen
    ? {
        width: fingerprintProfile.screen.width || device.viewport.width,
        height: fingerprintProfile.screen.height || device.viewport.height,
        deviceScaleFactor: fingerprintProfile.screen.deviceScaleFactor
  ...
  const hardwareConcurrency = fingerprintProfile?.navigator?.hardwareConcurrency || device.hardwareConcurrency;
  const deviceMemory = fingerprintProfile?.navigator?.deviceMemory || device.deviceMemory;
  ...
  const isMobileUA = /Android|iPhone|Mobile|iPad/i.test(userAgent);
  await alignUserAgentMeta(page, userAgent, {
    isMobile: isMobileUA,
    platformHint: fingerprintProfile?.navigator?.platform || device.platform
  });

  // 3) Navigator-level realism
  await setNavigatorOverrides(page, { ...device, viewport, hardwareConcurrency, deviceMemory });

  // 4) Stable media prefs (minimal, widely supported)
  await setStableMediaPrefs(page);

  // 5) Screen/DPR/orientation coherence
  await setScreenAndTouch(page, { viewport });
