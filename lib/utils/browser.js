...
async function enableSearchGeo(context, page, origin, { cdp: passedCdp } = {}) {
  ...
  } else if (context?.overridePermissions) {
    context.overridePermissions('https://www.google.com', ['geolocation']);
  }

  const cdp = passedCdp || (await createCdpSession(page));
...
async function prepareMobilePage(context, page, origin, fingerprintData) {
  // 1) Use the metadata from your generator to sync the network layer
  // fingerprintData comes from fingerprintGenerator.getFingerprint()
  if (fingerprintData?.userAgent) {
    await page.setUserAgent(fingerprintData.userAgent);
  }

  // 2) Keep your Location and Timezone logic (CRITICAL for Google)
  const cdp = await createCdpSession(page);
  
  // Match locale to fingerprint
  const locale = fingerprintData?.navigator?.language || 'en-US';
  await cdp.send('Emulation.setLocaleOverride', { locale });

  // Match timezone to your Proxy/Target location
...
async function createCdpSession(page) {
  ...
  throw new Error('Unable to create CDP session.');
}
