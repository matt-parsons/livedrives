...
async function parseLocalBusinesses(htmlInput, businessId = null) {
  const cheerio = require('cheerio');

  const html = Buffer.isBuffer(htmlInput)
    ? htmlInput.toString('utf8')
    : (typeof htmlInput === 'string' ? htmlInput : null);
  ...
  if (businessId != null) {
    try {
      await saveHtml(businessId, 1, 'testing', html);
    } catch (err) {
      note(`[parseLocalBusinesses] ⚠️ Failed to save HTML for ${businessId}: ${err.message}`);
    }
  }
  ...
  for (const selector of candidateSelectors) {
    const nodes = $(selector).toArray().filter((el) => {
      const $el = $(el);
      if ($el.is('a')) {
        if ($el.find('[role="heading"]').length) return true;
        const card = $el.closest('div.zPW3Kc, div[data-content-id]');
        return card.find('[role="heading"]').length > 0;
      }
      return $el.find('[role="heading"]').length > 0;
    });
    if (nodes.length) {
      cardNodes = nodes;
  ...
  if (!cardNodes.length) {
    note('[parseLocalBusinesses] ❌ No recognizable local business containers.');
    return [];
  }

  note(`[parseLocalBusinesses] Using selector "${activeSelector}" (${cardNodes.length} matches).`);
  ...
  const anchorSelectorJoined = anchorSelectors.join(', ');
  ...
  const isAdHref = (href) => {
    ...
    return /^https?:\/\/www\.google\.com\/aclk/i.test(value) ||
      value.startsWith('/aclk') ||
      value.includes('adurl=');
  };
  ...
  cardNodes.forEach((node, i) => {
    ...
    const $cardRoot = $candidate.is('.zPW3Kc')
      ? $candidate
      : $candidate.find('.zPW3Kc').first();
    const $scope = $cardRoot.length ? $cardRoot : $candidate;

    let $heading = $scope.find('[role="heading"][aria-level="3"]').first();
    if (!$heading.length) {
      $heading = $scope.find('[role="heading"]').first();
    }
    ...
    for (const selector of anchorSelectors) {
      $anchor = $scope.find(selector).first();
    ...
    if (!$anchor || !$anchor.length) {
      ...
      if (parentAnchor.length) {
        ...
        const siblingAnchor = $candidate.find(anchorSelectorJoined).first();
        if (siblingAnchor.length) $anchor = siblingAnchor;
      }
    }

    const rawHref = $anchor && $anchor.attr ? ($anchor.attr('href') || '').trim() : '';
    if (rawHref && isAdHref(rawHref)) {
      note(`  [${i}] ⚠️ Skipped: ad href detected (${rawHref}).`);
      return;
    }

    const normalizedHref = resolveHref(rawHref);
    const dedupeKey = `${name}::${normalizedHref || rawHref}`;
    if (seen.has(dedupeKey)) {
      note(`  [${i}] ⚠️ Skipped: duplicate entry detected.`);
      return;
    }
    seen.add(dedupeKey);
