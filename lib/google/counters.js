...
function parseLocalBusinesses(html) {
  ...
  const parseIdsFromHref = (rawHref) => {
    ...
    try {
      ...
      const qParam = url.searchParams.get('q');
      if (qParam && qParam.startsWith('place_id:')) {
        result.placeId = qParam.slice('place_id:'.length);
      }
      const cidParam = url.searchParams.get('cid') || url.searchParams.get('c');
      ...
      try {
        ...
        if (!result.cid) {
          const cidMatch = decoded.match(/cid=([0-9]+)/i);
          if (cidMatch) result.cid = cidMatch[1];
        }
        if (!result.placeId) {
          const placeMatch = decoded.match(/place_id[:=]([A-Za-z0-9_-]+)/i);
          if (placeMatch) result.placeId = placeMatch[1];
        }
        if (!result.mid) {
          const midMatch = decoded.match(/\/g\/([^/?]+)/i);
