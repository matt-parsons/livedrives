...
function parseLocalBusinesses(html) {
  ...
  const anchors = $('a[data-open-viewer]:not([data-ad-tracking-url])');
  console.log(`[parseLocalBusinesses] Found ${anchors.length} anchor candidates.`);

  const entries = [];

  anchors.each((i, el) => {
    const $anchor = $(el);
    const heading = $anchor.find('[role="heading"]').first();
    const name = (heading.text() || $anchor.text() || '').trim();

    if (!name) {
      console.log(`  [${i}] ⚠️ Skipped: no name text.`);
      return;
    }

    let href = $anchor.attr('href') || '';
    let mid = $anchor.attr('data-mid') || $anchor.attr('data-entityid') || $anchor.attr('data-entity-id') || '';
    ...
    for (let depth = 0; depth < 8 && $parent.length; depth++) {
      href = href || $parent.attr('href') || '';
      mid = mid || $parent.attr('data-mid') || $parent.attr('data-entityid') || $parent.attr('data-entity-id') || '';
    ...
    if (container.length) {
      const ratingNode = container.find('[aria-label*="star"], [aria-label*="Star"]').first();
      if (ratingNode.length) {
        const ratingMatch = ratingNode.attr('aria-label')?.match(/([0-9.]+)/);
      ...
      container.find('div, span').each((__, node) => {
