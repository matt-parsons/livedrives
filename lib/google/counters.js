...
async function parseLocalBusinesses(html) {
  ...
  businessCards.forEach((cardEl, i) => {
    ...
    if (!$anchor.length) {
      note(`  [${i}] ⚠️ Skipped: no anchor ancestor.`);
      return;
    }
    ...
    if (!normalizedHref) {
      note(`  [${i}] ⚠️ Skipped: missing href on anchor.`);
      return;
    }
    ...
    note(`  [${i}] ✅ Parsed: ${entry.name} | place_id=${entry.place_id || '—'} | cid=${entry.cid || '—'}`);
    entries.push(entry);
  });
  ...
  if (entries.length === 0) {
    ...
    console.log('[parseLocalBusinesses] HTML snippet preview:', snippet);
  } else {
