...
function parseLocalBusinesses(html) {
  const cheerio = require('cheerio');

  if (!html || typeof html !== 'string') {
    console.warn('[parseLocalBusinesses] ❌ Invalid HTML input:', typeof html);
    return [];
  }

  const $ = cheerio.load(html);
  console.log('[parseLocalBusinesses] ✅ HTML loaded. Length:', html.length);
  ...
  const parseIdsFromHref = (rawHref) => {
    ...
    try {
      ...
      console.warn('[parseIdsFromHref] ⚠️ URL parse failed:', rawHref, err.message);
    }
  ...
  console.log(`[parseLocalBusinesses] Found ${businessCards.length} <c-wiz> containers.`);
  ...
  businessCards.forEach((cardEl, i) => {
    ...
    if (!heading.length) {
      console.log(`  [${i}] ⚠️ Skipped: no heading found.`);
      return;
    }

    const name = heading.text().trim();
    if (!name) {
      console.log(`  [${i}] ⚠️ Skipped: empty heading text.`);
      return;
    }

    const $anchor = heading.closest('a');
    if (!$anchor.length) {
      console.log(`  [${i}] ⚠️ Skipped: no anchor ancestor.`);
      return;
    }
    ...
    if (!normalizedHref) {
      console.log(`  [${i}] ⚠️ Skipped: missing href on anchor.`);
      return;
    }

    if (isAdHref) {
      console.log(`  [${i}] ⚠️ Skipped: ad href detected (${normalizedHref}).`);
      return;
    }

    let mid =
      $anchor.attr('data-mid') ||
      $anchor.attr('data-entityid') ||
      $anchor.attr('data-entity-id') ||
      '';
    let placeId = $anchor.attr('data-place-id') || $anchor.attr('data-pid') || '';
    let cid = $anchor.attr('data-cid') || '';

    let $parent = $anchor.parent();
    for (let depth = 0; depth < 8 && $parent.length; depth++) {
      href = href || ($parent.attr('href') || '').trim();
      mid =
        mid ||
        $parent.attr('data-mid') ||
        $parent.attr('data-entityid') ||
        $parent.attr('data-entity-id') ||
        '';
      placeId = placeId || $parent.attr('data-place-id') || $parent.attr('data-pid') || '';
      cid = cid || $parent.attr('data-cid') || '';
      $parent = $parent.parent();
    }

    if (href) {
      const parsed = parseIdsFromHref(href);
      if (!placeId && parsed.placeId) placeId = parsed.placeId;
      if (!cid && parsed.cid) cid = parsed.cid;
      if (!mid && parsed.mid) mid = parsed.mid;
    ...
    const ratingNode = $card.find('[aria-label*="star"], [aria-label*="Star"]').first();
    if (ratingNode.length) {
      const ratingMatch = ratingNode.attr('aria-label')?.match(/([0-9.]+)/);
      if (ratingMatch) rating = Number(ratingMatch[1]);
      const reviewsMatch = ratingNode.attr('aria-label')?.match(/\(([\d,]+)\)/);
      if (reviewsMatch) reviews = Number(reviewsMatch[1].replace(/,/g, ''));
    }

    $card.find('div, span').each((__, node) => {
      const text = $(node).text().trim();
      if (!text) return;
      if (!snippet && /·/.test(text)) snippet = text;
      if (!address && /[0-9].*,/.test(text)) address = text;
    });
    ...
    const entry = {
      ...
      raw_place_id: placeId || mid || cid || null,
      cid: cid || null,
      mid: mid || null,
      url: href || null,
      rating: Number.isFinite(rating) ? rating : null,
      user_ratings_total: Number.isFinite(reviews) ? reviews : null,
      formatted_address: address || null,
      snippet: snippet || null
    };

    console.log(`  [${i}] ✅ Parsed: ${entry.name} | place_id=${entry.place_id || '—'} | cid=${entry.cid || '—'}`);
    entries.push(entry);
  });

  if (entries.length === 0) {
    console.warn('[parseLocalBusinesses] ❌ No business entries extracted.');
    const snippet = html.slice(500, 500).replace(/\s+/g, ' ');
    console.warn('[parseLocalBusinesses] HTML snippet preview:', snippet);
  } else {
    console.log(`[parseLocalBusinesses] ✅ Parsed ${entries.length} businesses.`);
  }
