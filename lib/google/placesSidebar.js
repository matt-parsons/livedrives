...
function extractSidebarFields(darray) {
  // assumes your getNested() helper exists in scope
  const categories =
    getNested(darray, 13) || // most common path (matches both dumps)
    [];

  const description =
    getNested(darray, 32, 1, 1) || // "From the business" description
    getNested(darray, 7, 1) ||     // older payloads
    getNested(darray, 7, 0, 1) ||
    null;

  const coverPhoto =
    getNested(darray, 218, 0, 0, 6, 0) || // primary image block (Star Painters)
    getNested(darray, 215, 0, 0, 6, 0) || // fallback (Quad Cities)
    null;

  return {
    categories: Array.isArray(categories)
      ? categories.filter((s) => typeof s === 'string' && s.trim())
      : [],
    description: typeof description === 'string' ? description.trim() : null,
    coverPhoto:
      typeof coverPhoto === 'string' && coverPhoto.startsWith('http')
        ? coverPhoto
        : null,
  };
}
...
// 3️⃣ Read the full APP_INITIALIZATION_STATE
...
// 4️⃣ Retry once if suspiciously small
if (!raw || raw.length < 150000) {
  console.warn('APP_INITIALIZATION_STATE looks small, reloading page for full data...');
...
const categories = getNested(bestArr, 13);
