...
async function fetchPlaceSidebarData(geometry, placeId, options = {}) {
  ...
  const fetchPromise = (async () => {
    ...
    logLine("INFO", 'fetchPlaceSidebarData', mapsUrl);
    let browser;
    ...
    try {
      ...
      try {
        ...
        page.on('request', (req) => {
          ...
          const blockedUrls = [
            ...
            // /\/maps\/preview\//i, this was blocking the correct redirect
      ...
      for (let attempt = 0; attempt <= maxAttempts; attempt++) {
        ...
        if (raw) {
          ...
          looksPartial =
            !raw ||
            raw.length < 70000 ||
            (Array.isArray(candidatesSrc) && candidatesSrc.length < 1);
        } else {
      ...
      for (const arr of parsedCandidates) {
        ...
        logLine("INFO", `Found Entry Name: ${entry.name}`);
      ...
      logLine("ERROR", 'Failed to fetch sidebar data');
      return {};
    } finally {
      if (browser) {
        try {
