...
const BACKGROUND_TIMEOUT_MS = 5 * 60 * 1_000;
...
async function fetchDataForSeoReviews(placeId, options = {}) {
  if (!placeId) {
    return { reviews: [], status: 'error', taskId: null };
  }
  ...
  try {
    if (!taskId) {
      ...
      if (createResponse.ok) {
        ...
        if (taskId && typeof options.onTaskCreated === 'function') {
    ...
    if (reviews.length > 0) {
      return { reviews, taskId, status: 'completed' };
    }
    ...
    return { reviews, taskId, status: reviews.length > 0 ? 'completed' : 'pending' };
  } catch (error) {
    console.error('Failed to fetch DataForSEO GBP reviews', error);
    return { reviews: [], taskId: taskId || null, status: 'error' };
  }
}

module.exports = {
  fetchDataForSeoReviews,
  fetchDataForSeoReviewsByTaskId,
