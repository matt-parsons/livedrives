...
async function clickSelectorsAndConfirm(
  ...
  const confirmSelectors = Array.isArray(confirmAppears)
    ? confirmAppears.filter((sel) => typeof sel === 'string' && sel.trim())
    : [];
  let confirmCountsBefore = null;
  let domBefore = null;

  if (confirmSelectors.length) {
    confirmCountsBefore = new Map();
    for (const sel of confirmSelectors) {
      try {
        const count = await page.locator(sel).count();
        confirmCountsBefore.set(sel, count);
      } catch {
        confirmCountsBefore.set(sel, 0);
      }
    }
  }

  if (domDelta && domDelta > 0) {
    try {
      domBefore = await page.evaluate(() => document.body?.innerText?.length || 0);
    } catch {
      domBefore = null;
    }
  }
  ...
  for (const sel of selectors) {
    ...
    if (confirmSelectors.length || domBefore !== null) {
      const start = Date.now();
      while (Date.now() - start < settle) {
        for (const confirmSel of confirmSelectors) {
          try {
            const count = await page.locator(confirmSel).count();
            const beforeCount = confirmCountsBefore?.get(confirmSel) || 0;
            if (count > beforeCount) {
              const matchedSelector = typeof sel === 'object' ? `text:${sel.text}` : sel;
              return {
                success: true,
                via: 'confirm-appeared',
                matchedSelector,
                confirmSelector: confirmSel
              };
            }
          } catch {}
        }
        ...
        if (domBefore !== null) {
          try {
            const domAfter = await page.evaluate(() => document.body?.innerText?.length || 0);
            if (Math.abs(domAfter - domBefore) >= domDelta) {
              const matchedSelector = typeof sel === 'object' ? `text:${sel.text}` : sel;
              return { success: true, via: 'dom-delta', matchedSelector };
            }
          } catch {}
        }
...
try {
  const rankResult = await page.evaluate(({ businessName, mid }) => {
    ...
    // Try MID-specific selectors first
    const selectors = [
      ...
      // Fallback to generic selectors if MID-specific ones don't work
      'a[data-open-viewer]:not([data-ad-tracking-url])',
      'a[href*="/maps/place/"]'
    ];
    ...
    for (const selector of selectors) {
      try {
        ...
        if (element) {
          matchedAnchor = element;
          usedSelector = selector;
          break;
        }
      } catch (e) {
        // Selector might be invalid if mid contains special characters, skip it
        continue;
      }
    }
    ...
    if (!matchedAnchor) {
      return { 
        ...
        debug: {
          searchedMid: mid,
          totalDataOpenViewer: document.querySelectorAll('a[data-open-viewer]').length
        }
      };
    }

    // Now find all non-ad local pack entries to determine rank
    ...
    if (allAnchors.length === 0) {
      return {
        rank: 1, // If we found the business but can't determine rank, assume position 1
        via: 'found_but_no_pack',
        matched: matchedAnchor.textContent?.trim() || 'unknown',
        selector: usedSelector
      };
    }

    // Find the position of our matched anchor in the list
    const rankIndex = allAnchors.indexOf(matchedAnchor);
    ...
    if (rankIndex !== -1) {
      return {
        ...
        matched: matchedAnchor.textContent?.trim() || 'unknown',
        selector: usedSelector,
        totalInPack: allAnchors.length
      };
    }

    // If the matched anchor isn't in the pack list, it might be a different element
    // Try to find it by name in the pack
    const entries = allAnchors.map((anchor, index) => {
      const heading = anchor.querySelector('[role="heading"]');
      const ariaLabel = anchor.getAttribute('aria-label');
      const textContent = heading?.textContent || ariaLabel || anchor.textContent || '';
    ...
    const viaName = entries.find((entry) =>
      targetName && entry.name && (entry.name === targetName || entry.name.includes(targetName) || targetName.includes(entry.name))
    );
    ...
    if (viaName) {
      return {
        rank: viaName.index,
        via: 'name_fallback',
        matched: viaName.element.textContent?.trim() || 'unknown',
        totalInPack: allAnchors.length
      };
    }
    ...
    return { 
      rank: null, 
      via: 'found_element_but_not_in_pack',
      matched: matchedAnchor.textContent?.trim() || 'unknown',
      selector: usedSelector
    };
  }, { businessName, mid });
  ...
  if (rankResult && typeof rankResult.rank === 'number') {
    ...
    note(`→ [CTR] Located business at position ${rank}/${rankResult.totalInPack || '?'} (${rankResult.via}: ${rankResult.matched || ''})`);
  } else {
    note(`→ [CTR] Business rank not found (${rankResult.via}${rankResult.matched ? ': ' + rankResult.matched : ''})`);
    if (rankResult.debug) {
      note(`→ [CTR] Debug: ${JSON.stringify(rankResult.debug)}`);
    }
  }
} catch (err) {
