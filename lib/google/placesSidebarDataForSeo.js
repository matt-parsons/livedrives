...
const DATAFORSEO_POSTS_LIVE_ENDPOINT =
  'https://api.dataforseo.com/v3/business_data/google/my_business_updates/live';
...
async function fetchPlaceSidebarDataForSeo(placeId, options = {}) {
  ...
  try {
    const postsResponse = await fetch(DATAFORSEO_POSTS_LIVE_ENDPOINT, {
      method: 'POST',
      headers,
      body: JSON.stringify(payload),
    });
    ...
    if (postsResponse.ok) {
      ...
      const postItems = postsData?.tasks?.[0]?.result?.[0]?.items || [];
      posts = postItems
        .map((post) => (post ? post.post_date : null))
        .filter(Boolean);
    } else {
      console.error(`DataForSEO posts request failed with status ${postsResponse.status}`);
    }
  } catch (error) {
