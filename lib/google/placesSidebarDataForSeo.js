...
const DATAFORSEO_POSTS_TASKS_READY_ENDPOINT =
  'https://api.dataforseo.com/v3/business_data/google/my_business_updates/tasks_ready';
...
const DEFAULT_POSTS_POLL_INTERVAL_MS = 5_000;
const DEFAULT_POSTS_TIMEOUT_MS = 70_000;
...
async function fetchPlaceSidebarDataForSeo(placeId, options = {}) {
  ...
  const posts = await fetchDataForSeoPosts(placeId, headers, options);

  return mapDataForSeoItem(resultItem, placeId, posts);
}

async function fetchDataForSeoPosts(placeId, headers, options = {}) {
  const pollIntervalMs = Number(options?.postsPollIntervalMs) || DEFAULT_POSTS_POLL_INTERVAL_MS;
  const timeoutMs = Number(options?.postsTimeoutMs) || DEFAULT_POSTS_TIMEOUT_MS;
  ...
  try {
    ...
    while (Date.now() < deadline) {
      const readyTaskId = await waitForDataForSeoPostsTaskReady(taskId, headers);

      if (readyTaskId) {
        posts = await fetchDataForSeoPostsByTaskId(readyTaskId, headers);
        if (posts.length > 0) {
          break;
        }
      }
...
async function waitForDataForSeoPostsTaskReady(taskId, headers) {
  try {
    const readyResponse = await fetch(DATAFORSEO_POSTS_TASKS_READY_ENDPOINT, {
      method: 'POST',
      headers,
      body: JSON.stringify([]),
    });
    ...
    if (!readyResponse.ok) {
      console.error(`DataForSEO posts tasks_ready failed with status ${readyResponse.status}`);
      return null;
    }

    const readyData = await readyResponse.json();
    const readyTasks = (readyData?.tasks ?? []).flatMap((task) => task?.result ?? [task]);

    const matchingTask = readyTasks.find((task) => task?.id === taskId || task?.id_task === taskId);
    return matchingTask ? taskId : null;
  } catch (error) {
    console.error('DataForSEO posts tasks_ready check failed', error);
    return null;
  }
}
