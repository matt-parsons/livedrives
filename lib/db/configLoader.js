...
function rowToConfig(row) {
  ...
  return {
    ...
    soax: {
      ...
      res_username: row.soax_res_username_db || row.soax_res_username || '',
      password: row.soax_password || process.env.SOAX_PASSWORD || '',
      res_password: row.soax_res_password || process.env.SOAX_PASSWORD_RES || ''
    },
    ...
    const [bizRows] = await conn.query(
      `
        SELECT
          ...
          sc.res_username AS soax_username_db
        FROM businesses b
        LEFT JOIN business_hours h
          ON h.business_id = b.id
        LEFT JOIN (
          SELECT c.business_id, c.endpoint, c.res_username
          FROM soax_configs c
          JOIN (
        ...
        WHERE b.is_active = 1
          AND h.windows_json IS NOT NULL
      `
    );
    ...
    const [rows] = await conn.query(
      `
        SELECT
          ...
          sc.res_username AS soax_res_username_db
        FROM businesses b
        LEFT JOIN business_hours h
          ON h.business_id = b.id
        LEFT JOIN (
          SELECT c.business_id, c.endpoint, c.username, c.res_username
          FROM soax_configs c
          JOIN (
            SELECT business_id, MAX(created_at) AS max_created
            FROM soax_configs
