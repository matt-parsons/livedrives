// lib/db/ctr_store.js
// Inserts into `ranking_queries` and `ranking_snapshots` using mysql2/promise pool.
...
let snapshotInsertMode = 'unknown'; // 'with-business' | 'no-business'

async function insertSnapshotRow(conn, {
  runId,
  queryId,
  businessId,
  originLat,
  ...
  const tryWithBusiness = snapshotInsertMode !== 'no-business';
  const normalizedRunId = runId != null ? Number(runId) : null;
  const normalizedBusinessId = businessId != null ? Number(businessId) : null;

  if (tryWithBusiness) {
    try {
      const sql = `
        INSERT INTO ranking_snapshots
          (run_id, query_id, business_id, origin_lat, origin_lng, total_results, matched_place_id, matched_position, results_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
      const params = [
        normalizedRunId,
      ...
      snapshotInsertMode = 'with-business';
      return;
    } catch (err) {
      if (err?.code !== 'ER_BAD_FIELD_ERROR' && err?.errno !== 1054) throw err;
      console.warn('[ctr_store] ranking_snapshots.business_id missing, falling back without column');
      snapshotInsertMode = 'no-business';
      // fall through to retry without business_id
    }
  }

  const sql = `
    INSERT INTO ranking_snapshots
      (run_id, query_id, origin_lat, origin_lng, total_results, matched_place_id, matched_position, results_json)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `;
  const params = [
    normalizedRunId,
...
async function recordRankingSnapshot(args) {
  ...
  try {
    ...
    if (placesWithMatchFlags.length) {
      await insertSnapshotRow(conn, {
        runId,
        queryId,
        businessId,
        originLat,
        originLng,
        totalResults: placesWithMatchFlags.length,
        matchedPlaceId: matchedPlaceIdForQuery,
        matchedPosition: matchedPositionForQuery,
        resultsJson: JSON.stringify(placesWithMatchFlags)
      });
    }
