...
function hasDefinedHours(hours) {
  if (!hours || typeof hours !== 'object') {
    return false;
  }

  return Object.values(hours).some((segments) => {
    if (Array.isArray(segments)) {
      return segments.some(Boolean);
    }
...
function buildScheduleConfig(business, hours) {
  ...
  return {
    ...
    business_hours: hasDefinedHours(hours) ? hours : fallbackHours
  };
}
...
async function replaceScheduleKeywords(businessId, keywords, options = {}) {
  ...
  try {
    ...
    if (normalized.length) {
        const placeholders = normalized.map(() => '(?, ?, UTC_TIMESTAMP())').join(', ');
        const flatValues = normalized.flatMap((keyword) => [businessId, keyword]);

        await connection.query(
          `INSERT INTO geo_grid_schedule_keywords (business_id, keyword, created_at) VALUES ${placeholders}`,
          flatValues
        );
    }
...
async function markScheduleRunComplete(businessId, executedAt, options = {}) {
  ...
  try {
    ...
    if (!nextSlot) {
      // Fallback: If no slot is found, log a warning and schedule for 24 hours later.
      // This prevents the schedule from being permanently disabled by a NULL next_run_at.
      console.warn(`[scheduler] Could not determine next run slot for business ${businessId}. Retrying in 7 days.`);
      nextSlot = executed.plus({ days: 7 });
    }
...
function resolveReferenceDate(reference, timezone = 'UTC') {
  if (reference && typeof reference.toISO === 'function') {
    return reference.setZone(timezone);
  }

  if (reference instanceof Date) {
    return DateTime.fromJSDate(reference, { zone: 'utc' }).setZone(timezone);
  }

  if (typeof reference === 'string') {
    const parsed = DateTime.fromISO(reference, { zone: 'utc' });
    if (parsed.isValid) {
...
async function scheduleNextRunAfter(businessId, referenceDate, options = {}) {
  const { connection: pendingConnection, release } = ensureConnection(options.connection || null);
  const connection = await pendingConnection;
  ...
  try {
    ...
    if (!context.schedule) {
      await initializeGeoGridSchedule(businessId, { connection, reference: referenceDate || options.reference });
      context = await loadScheduleContext(businessId, connection);
    }
    ...
    const reference = resolveReferenceDate(referenceDate || options.reference, context.business.timezone);
    const config = buildScheduleConfig(context.business, context.businessHours);

    let nextSlot = nextOccurrenceForSchedule(config, context.schedule, { reference });

    if (!nextSlot) {
      nextSlot = findNextSlot(config, {
        reference,
        minLeadMinutes: context.schedule.minLeadMinutes
      });
    }

    if (!nextSlot) {
      console.warn(`[scheduler] Could not determine next run slot for business ${businessId}. Retrying in 7 days.`);
      nextSlot = reference.plus({ days: 7 });
    }
