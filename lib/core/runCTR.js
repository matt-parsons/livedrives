...
const { createInjectedPage } = require('../utils/fingerprint');
...
module.exports = async function runCTR({ runId, config, keyword, origin, sessionId, run2Captcha }) {
  ...
  try {
    ...
    try {
      const result = await createInjectedPage({ browser });
      page = result.page;
...
page.on('response', async (response) => {
  ...
  try {
    ...
    if (html) {
      ...
      await takeScreenshot(page, 'empty_body_error', config.company_id);
      note(`→ [CTR] FAILED: Response was valid but body was empty/unreadable.`);
    }

  } catch (err) {
    await takeScreenshot(page, 'failed_serp_response', config.company_id);
    ...
    await serpMicroInteractions(page, { minDwellMs: 800, maxDwellMs: 1600 });
    await takeScreenshot(page, 'typed-search', config.company_id);
