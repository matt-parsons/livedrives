...
const { tzFromLatLng, enableSearchGeo } = require('../utils/browser');
const { createInjectedPage, generateFingerprint } = require('../utils/fingerprint');
...
module.exports = async function runCTR({ runId, config, keyword, origin, sessionId, run2Captcha }) {
  ...
  const fingerprintWithHeaders = generateFingerprint();

  rank = null;

  const soaxConfig = { ...config.soax, sessionId };
  let username, password, endpoint, ip, city;
  try {
    ({ username, password, endpoint, ip, city } = await getSoaxProxyAuth(soaxConfig));
  } catch (error) {
    console.error('→ [CTR] Failed to obtain SOAX proxy for CTR run:', error.message);
    return quit('soax_failed'); // Cancel the run
  }
  ...
  const userDataDir = `/tmp/playwright-${sessionId || Date.now()}`;
  if (!fs.existsSync(userDataDir)) fs.mkdirSync(userDataDir);
  
  const { width, height } = fingerprintWithHeaders.fingerprint.screen;
  ...
  let hasLoggedHtmlCapture = false;
  ...
  try {
    ...
    try {
      const result = await createInjectedPage({ context, fingerprintWithHeaders });
      page = result.page;
    ...
    await page.route('**/*', (route) => {
      ...
      // These specific subdomains serve the heavy thumbnails in Map Packs
      if (url.includes('lh3.googleusercontent.com') || url.includes('books.google.com/books/content')) {
        return route.abort();
      }
      
      // Abort fonts - saves about 500kb-1mb per load with no bot risk
      if (route.request().resourceType() === 'font') {
        return route.abort();
      }
      
      if (url.includes('googleusercontent.com') || url.includes('/vt/pb=')) {
        return route.abort();
      }      
    ...
    page.on('response', async (response) => {
      const headers = response.headers();
        if (headers['content-length']) {
          totalKiloBytes += parseInt(headers['content-length'], 10) / 1024;
        }
      });
      note(`→ [CTR] Total data downloaded: ${totalKiloBytes.toFixed(2)} KB`);

      const url = response.url();
      
      // Only log for search-related URLs to avoid spamming the console
      ...
      try {
        ...
        // console.log(`→ [DEBUG-NET] URL: ${url.substring(0, 60)}...`);
        // console.log(`→ [DEBUG-NET] Status: ${status} | Type: ${resourceType} | Content: ${contentType}`);

        // 1. Check for Blocks (Status 403 or 429)
        if (status !== 200) {
          if (status === 403 || status === 429) {
            note(`→ [CTR] Google Blocked Request (Status ${status}) on ${url.substring(0, 40)}`);
          }
          return;
        }

        // 2. Check for Resource Type
        if (resourceType !== 'document' || !contentType.includes('text/html')) {
          return; 
        }

        if (!hasLoggedHtmlCapture) {
          note(`→ [CTR] Valid Search HTML detected. Attempting to read body...`);
        }

        // 3. SAFE CAPTURE: The .catch prevents the "Cannot read properties of undefined" crash
        // if the connection is dropped by Google while reading.
        // const html = await response.text().catch((e) => {
        //   note(`→ [CTR] CRITICAL: Connection dropped while reading body: ${e.message}`);
        //   return null;
        // });
        ...
        if (html) {
          ...
          if (!hasLoggedHtmlCapture) {
            note(`→ [CTR] SUCCESS: latestSearchHtml updated (${html.length} chars)`);
            hasLoggedHtmlCapture = true;
          }
        } else {
          // This is where your screenshot error was born
          await takeScreenshot(page, 'empty_body_error', config.business_id);
          note(`→ [CTR] FAILED: Response was valid but body was empty/unreadable.`);
        }

      } catch (err) {
        // if (!hasLoggedHtmlCapture) {
          // await takeScreenshot(page, 'failed_serp_response', config.business_id);
          note(`→ [CTR] Unexpected Listener Error: ${err.message}`);
        // }
      }
    });
    ...
    console.log(`→ [CTR] SOAX proxy configured`);
    console.log(`→ [CTR] Launched with device "${device.name}"`);

    const cdp = await context.newCDPSession(page);
    const tz = tzFromLatLng(origin.snapped_location.lat, origin.snapped_location.lng);
    await cdp.send('Emulation.setTimezoneOverride', { timezoneId: tz });
    await enableSearchGeo(context, page,
      { lat: origin.snapped_location.lat, lng: origin.snapped_location.lng },
      { cdp }
    );
    ...
    try{
      // await page.locator('textarea[name="q"]').click({ force: true });
      await page.click('textarea[name="q"], .search_button_suggest');
    } catch {
      await takeScreenshot(page, 'activate_search_error', config.business_id);
    }
    ...
    await serpMicroInteractions(page, { minDwellMs: 800, maxDwellMs: 1600 });
    await takeScreenshot(page, 'typed-search', config.business_id);

    await page.keyboard.press('Enter');
    console.log(`→ [CTR] Searching Google`);
    // try { await page.waitForNavigation({ waitUntil: 'networkidle', timeout: 3000 }); }
    try { await page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 3000 }); }
    ...
    try {
      ...
      await takeScreenshot(page, 'captcha_error', config.business_id);
    }
    ...
    await takeScreenshot(page, 'finding-business', config.business_id);

    try {
      // await page.waitForSelector('a[data-open-viewer]:not([data-ad-tracking-url])', { timeout: 12000 });
      await page.locator('a[data-open-viewer]:not([data-ad-tracking-url])').waitFor({ timeout: 12000 });
    } catch (_) {
    ...
    if (!found) {
      ...
      await takeScreenshot(page, 'not_found', config.business_id);

      const { dismissAppPrompt } = require('../google/appPrompt');
      await dismissAppPrompt(page, config.business_id);
        ...
        try {
          if (currentUrl.includes('/sorry/')) {
            ...
            while (attempts2 < 3 && !success2) {
              ...
              if (!success2) { note(`→ [CTR] 2nd CAPTCHA Unsolved`); await delay(5000); }
            }
          ...
          await takeScreenshot(page, '2nd_captcha_error', config.business_id);
        }
        ...
        if (clickedMore) {
          try {
            ...
            if (found) {
              ...
              await takeScreenshot(page, `clicked_more_businesses_error_${attempts}`, config.business_id);
            }
          } catch (e) {
          ...
          await takeScreenshot(page, `clicked_more_button_missing_${attempts}`, config.business_id);
    ...
    if (found) {
      ...
      await waitForFullLoad(page);
      await humanScroll(page);
      await require('../google/appPrompt').dismissAppPrompt(page, config.business_id);
      await humanDelay();
      await humanScroll(page);
      reason = 'success';
      await takeScreenshot(page, 'success', config.business_id);
      try {
        try {
      ...
      await takeScreenshot(page, 'failed', config.business_id);
    }
