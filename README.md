...
- The dashboard starts by requesting a fresh posts task via `GET /api/optimization-data?resetPostsTask=1&forceRefresh=1`, then polls the latest task id.
- If polling exceeds the timeout window, it requests another posts task via `GET /api/optimization-data?resetPostsTask=1&forceRefresh=1` and stops polling after repeated timeouts.
